...
AS
	...
	SELECT
		...
		ISNULL(c.last_name,'') + 
		CASE 
			WHEN ISNULL(c.first_name,'')<>'' THEN 
				', ' + ISNULL(c.first_name,'') 
			ELSE 
				'' 
		END + 
		' ' + 
		ISNULL(c.middle_initial,'') + 
		CASE 
			WHEN LEN(ISNULL(c.first_name,'') + ISNULL(c.last_name,'') + ISNULL(c.middle_initial,''))>0 THEN 
				char(13) + char(10) 
			ELSE 
				'' 
		END + 
		ISNULL(c.company_name,''))) AS customer_name,
		tc.amount AS amount
	FROM
